...
import { Physics, RigidBody, BallCollider, CuboidCollider, useRopeJoint, useSphericalJoint, RigidBodyProps } from '@react-three/rapier';
import { useEffect, useRef, useState } from 'react';
import * as THREE from 'three';
import DecryptedText from './DecryptedTextComp.tsx';
    ...
    <section className="w-full h-screen bg-black text-white relative">
      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-center p-6">
        <h1 className="text-4xl md:text-6xl font-bold mb-4">
          <DecryptedText
            text="Desenvolvimento que conecta marcas e tecnologia."
            speed={45}
            sequential
        ...
        <p className="text-lg md:text-xl max-w-2xl mt-2">
          <DecryptedText
            text="Seu site não é só código. É sua vitrine, sua voz, sua vantagem."
            speed={30}
            sequential
            revealDirection="start"
            animateOn="view"
      ...
      <Canvas camera={{ position: [0, 0, 30], fov: 20 }} className="absolute inset-0 z-0">
...
function CordaoHero({ maxSpeed = 50, minSpeed = 0 }: { maxSpeed?: number; minSpeed?: number }) {
  ...
  return (
    <group position={[0, 4, 0]}>
      ...
      <RigidBody ref={card} position={[2, 0, 0]} {...segmentProps} type={dragged ? 'kinematicPosition' : 'dynamic'}>
        ...
        {/* CARD VISUAL REMOVIDO COMPLETAMENTE */}
      </RigidBody>
    </group>
  );
}
